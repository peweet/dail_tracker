...
User question: **What is the public accountability profile of this member across approved datasets?**
...
```text
v_member_overview, v_member_profile_summary, v_member_dataset_presence
```
...
6. `utility/pages_code/member_overview.py`
...
```text
Create a bold UI redesign plan for `member_overview`.
...
```text
Update `utility/page_contracts/member_overview.yaml` only if the redesign plan requires contract changes.
...
```text
Implement the `member_overview` UI redesign.

This is a UI redesign, not a safe refactor.

You may modify only the allowed files listed in `page_runbooks/member_overview.md`.
...
```text
The redesign is still too close to the old `member_overview` page.

...
index of every public pipeline data source: its refresh cadence, when it is next due, the
...
stale — the registry is past what anyone can reliably eyeball.
...
## The registry map — what is authoritative

The public cadence ledger indexes the canonical generated registry. Private product
overlays keep separate registries and cadence metadata and are deliberately not copied
into this public checkout.

| Registry | Rows | Role | In the ledger? |
|---|---:|---|---|
| `data/_meta/source_registry.generated.json` | generated | **Canonical public live pipeline sources**, generated from in-code configs | **Yes** (`registry=source_registry`) |
| Private product source registries | private overlay | Professional-review product sources and cadence | No — remain private |
...
The seeds feed the canonical registry; sandbox lists are pre-promotion targets and do
...
- **test_packages** — a rough "which tests to run when this source changes" hint,
  ...
  and makes thin coverage visible.

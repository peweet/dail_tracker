...
def test_source_unavailable_returns_cleanly(monkeypatch):
    ...
    _patch(
        monkeypatch, supplier=QueryResult.success(pd.DataFrame()), stats=QueryResult.unavailable("test: view missing")
    )
        ...
        [
            {
                ...
                "n_awards": 5,
                "n_authorities": 2,
                "awarded_value_safe_eur": 1_500_000.0,
                "company_num": "123456",
                "company_status": "Normal",
                "cro_match_method": "exact",
                "on_lobbying_register": True,
                "lobbying_returns": 3,
                "is_lobbying_registrant": False,
                "is_lobbying_client": True,
            },
            {
                "supplier": "Beta Services Ltd",
                "supplier_norm": "beta services ltd",
                "n_awards": 2,
                "n_authorities": 1,
                "awarded_value_safe_eur": 40_000.0,
                "company_num": pd.NA,
                "company_status": pd.NA,
                "cro_match_method": pd.NA,
                "on_lobbying_register": pd.NA,
                "lobbying_returns": pd.NA,
                "is_lobbying_registrant": pd.NA,
                "is_lobbying_client": pd.NA,
            },
        ]
    )

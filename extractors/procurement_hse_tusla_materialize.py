...
PUBS = {
    "ie_hse": {
        "name": "Health Service Executive", "ptype": "state_body", "sector": "health",
        "semantics": "payment_actual", "privacy": "low",
    ...
    "ie_tusla": {
        "name": "Tusla – Child and Family Agency", "ptype": "agency", "sector": "social",
        "semantics": "po_committed", "privacy": "medium",
...
def to_schema(pid: str, native: dict, file_url: str, fhash: str, semantics: str, meta: dict) -> dict:
    ...
    return {
        "publisher_id": pid, "publisher_name": meta["name"],
        "publisher_type": meta["ptype"], "sector": meta["sector"],
        "source_landing_url": meta["landing"], "source_file_url": file_url,
        "source_file_hash": fhash, "period": period, "year": y, "quarter": quarter,
        "supplier_raw": native["supplier_raw"], "amount_eur": native["amount_eur"],
        "amount_semantics": semantics, "description": native.get("description"),
        "po_number": native.get("doc_ref") or None, "paid_flag": None,
        "source_row_number": native.get("source_row"), "source_page_number": native.get("source_page"),
        "parser_name": f"hse_tusla_xcut_{pid}", "parser_version": PARSER_VERSION,
...
def main() -> None:
    ...
    for pid, meta in PUBS.items():
        for url, cached in meta["files"]:
            ...
            if not ok:
                ...
                per_file.append({"publisher": pid, "file": tag, "status": "excluded", "reason": why,
                                 "rows": len(native)})
                continue
            ...
            per_file.append({"publisher": pid, "file": tag, "status": "ok",
    ...
    for pid in PUBS:
        ...
        if sub.height:
            print(f"  {pid:<9} rows={sub.height:>6}  €{sub['amount_eur'].sum():>15,.0f}  "

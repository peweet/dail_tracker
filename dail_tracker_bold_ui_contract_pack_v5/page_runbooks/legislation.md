# Page Runbook — Legislation

## Target

Page ID: `legislation`  
Page file: `utility/pages_code/legislation.py`  
User question: **What legislation is visible in the dataset, and how can users inspect its official records?**
...
```text
v_legislation_index, v_legislation_detail, v_legislation_timeline, v_legislation_sources
```
...
- `utility/page_contracts/legislation.yaml`
...
```text
We are working on `legislation` only.

Read only:
1. `CLAUDE.md`
2. `page_runbooks/legislation.md`
3. `utility/page_contracts/legislation.yaml`
4. `utility/page_contracts/_shared_ui_policy.yaml`
5. `utility/page_contracts/_interaction_patterns.yaml`
6. `utility/pages_code/legislation.py`
...
```text
Create a bold UI redesign plan for `legislation`.
...
```text
Update `utility/page_contracts/legislation.yaml` only if the redesign plan requires contract changes.
...
```text
Implement the `legislation` UI redesign.

This is a UI redesign, not a safe refactor.

You may modify only the allowed files listed in `page_runbooks/legislation.md`.
...
```text
The redesign is still too close to the old `legislation` page.

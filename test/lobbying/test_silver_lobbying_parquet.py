...
class MostLobbiedPoliticiansSchema(pa.DataFrameModel):
    ...
    def warn_on_unexpected_columns(cls, data) -> bool:
        declared = {"full_name", "chamber", "lobby_returns_targeting", "distinct_orgs", "total_returns"}
        extra = set(_df(data).columns) - declared
        if extra:
            import warnings

            warnings.warn(f"Unexpected columns in most_lobbied_politicians: {extra}", stacklevel=2)
        return True
...
SAMPLE_RETURNS = pl.DataFrame(
    {
        ...
        "lobbying_period_start_date": pl.Series([datetime(2023, 1, 1), datetime(2023, 7, 1)], dtype=pl.Datetime),
        "lobbying_period_end_date": pl.Series([datetime(2023, 3, 31), datetime(2023, 9, 30)], dtype=pl.Datetime),
        "lobby_url": [
...
SAMPLE_RETURNS_DUPLICATE_PK = pl.DataFrame(
    {
        ...
        "was_this_a_grassroots_campaign": [False, False],
        "lobbying_period_start_date": pl.Series([datetime(2023, 1, 1), datetime(2023, 1, 1)], dtype=pl.Datetime),
        "lobbying_period_end_date": pl.Series([datetime(2023, 3, 31), datetime(2023, 3, 31)], dtype=pl.Datetime),
        "lobby_url": [
            "https://www.lobbying.ie/return/10001",
            "https://www.lobbying.ie/return/10001",
        ],
...
SAMPLE_BILATERAL = pl.DataFrame(
    {
        ...
        "relationship_start": pl.Series([datetime(2020, 1, 1), datetime(2021, 6, 1)], dtype=pl.Datetime),
        "relationship_last_seen": pl.Series([datetime(2023, 6, 1), datetime(2023, 9, 1)], dtype=pl.Datetime),
    }
)
...
LOBBYING_TABLES = [
    ...
    ("revolving_door_returns_detail.parquet", "dpos_or_former_dpos_who_carried_out_lobbying_name"),

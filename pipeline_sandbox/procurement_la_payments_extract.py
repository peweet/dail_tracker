...
COMPANY_SUFFIX = re.compile(
    ...
    r"council|board|institute|ireland|technolog|systems|media|construction|contractors|&)\b", re.I)
...
PUBLIC_BODY = re.compile(
    ...
    r"local authority|\bOPW\b|hospital|\birish water\b|uisce)\b", re.I)
ID_CODE_RE = re.compile(r"^[\d\W]+$|^[A-Z]?\d{4,}$")  # Fingal: supplier published as a bare code


# ============================================================================ CONFIG
def la(slug, council, region, entity, *, fmt, listing, value_kind,
       direct=None, include=None, neg_abs=False, supplier_is_id=False,
       aggregate_guard=False, status="READY", caveat="") -> dict:
    """One local authority. value_kind ∈ {po_committed (→COMMITTED), payment_actual (→SPENT)}."""
    return {
        "slug": slug, "council": council, "region": region, "entity": entity,
        "fmt": fmt, "listing_url": listing, "value_kind": value_kind,
        "direct_files": direct or [], "include": re.compile(include, re.I) if include else None,
        "neg_abs": neg_abs, "supplier_is_id": supplier_is_id,
        "aggregate_guard": aggregate_guard, "status": status, "caveat": caveat,
    }
...
SCHEMA_MAP: list[dict] = [
    ...
    la("south_dublin", "South Dublin", "Dublin", "dublin", fmt="xlsx",
       listing="https://www.sdcc.ie/en/services/business/payments/", value_kind="po_committed",
       caveat="PO list carries a PAID Y/N flag (carried per row)"),
    la("cork_city", "Cork City", "Munster", "city", fmt="xlsx",
       listing="https://www.corkcity.ie/en/council-services/public-info/spending-and-revenue/",
       value_kind="po_committed", caveat="xlsx; old media-folder PDFs 404"),
    la("wicklow", "Wicklow", "Leinster", "county", fmt="xlsx",
       listing="https://www.wicklow.ie/Living/Your-Council/Finance/Procurement/Purchase-Orders-Over-20-000",
       value_kind="po_committed"),
    la("monaghan", "Monaghan", "Ulster", "county", fmt="xlsx",
       listing="https://monaghan.ie/finance/publication-of-purchase-orders/", value_kind="po_committed"),
    la("kilkenny", "Kilkenny", "Leinster", "county", fmt="xlsx",
       listing="https://kilkennycoco.ie/eng/services/finance/purchase-orders-over-%E2%82%AC20-000/",
       value_kind="po_committed", neg_abs=True,
       caveat="amounts stored NEGATIVE (debit sign) → abs(); supplier col 'Ap/Ar ID(T)' resolved by content-fallback"),
    la("wexford", "Wexford", "Leinster", "county", fmt="xlsx",
       listing="https://www.wexfordcoco.ie/council-and-democracy/procurement-finance-and-credit-control/council-spend",
       value_kind="po_committed", caveat="legacy .xls (~9%) skipped (needs xlrd)"),
    # ---- digital PDF (fitz largest-x-gap; NO OCR) ----
    la("cork_county", "Cork County", "Munster", "county", fmt="pdf",
       listing="https://www.corkcoco.ie/en/council/accessibility-maps-and-publications/purchase-orders-in-excess-of-eu20000",
       value_kind="po_committed",
       direct=["https://www.corkcoco.ie/sites/default/files/2025-08/2025-q2-purchase-orders-in-excess-of-eu20000.pdf",
    ...
    la("kildare", "Kildare", "Leinster", "county", fmt="pdf",
       listing="https://kildarecoco.ie/YourCouncil/Publications/Finance/PurchaseOrdersover20000/", value_kind="po_committed"),
    la("westmeath", "Westmeath", "Leinster", "county", fmt="pdf",
       listing="https://www.westmeathcoco.ie/en/ourservices/finance/procurement/purchaseorders/", value_kind="po_committed"),
    la("waterford", "Waterford", "Munster", "merged", fmt="pdf",
       ...
       value_kind="po_committed", caveat="leading OrderNo column → digit-prefix strip"),
    la("limerick", "Limerick", "Munster", "merged", fmt="pdf",
       listing="https://www.limerick.ie/council/services/business-and-economy/revenue-collection/accounts-payable",
       value_kind="po_committed",
       direct=["https://www.limerick.ie/sites/default/files/media/documents/2026-05/purchase-orders-over-eu20-000-quarter-1-2026.pdf"],
       caveat="row carries a Paid column"),
    la("offaly", "Offaly", "Leinster", "county", fmt="pdf",
       listing="https://www.offaly.ie/financial-reports/", value_kind="payment_actual",
       include=r"payment|20k|over.?20|greater", caveat="GL30 'Payments Greater than €20k' → SPENT grain"),
    la("longford", "Longford", "Leinster", "county", fmt="pdf",
       listing="https://www.longfordcoco.ie/services/finance/finance-documents/large-purchase-orders/", value_kind="po_committed"),
    la("galway_city", "Galway City", "Connacht", "city", fmt="pdf",
       listing="https://www.galwaycity.ie/services/finance-services/budgets-and-financial-publications",
       value_kind="po_committed",
       direct=["https://www.galwaycity.ie/sites/default/files/2026-05/Qtr%201%202026_Purchase%20Orders%20over%20%E2%82%AC20k_0.pdf"],
       caveat="amount is the LAST column (handled by rightmost-money pick); budgets page mixes prompt-pay"),
    la("galway_county", "Galway County", "Connacht", "county", fmt="pdf",
       listing="https://www.gaillimh.ie/en/finance/financial-publications/purchase-orders", value_kind="po_committed",
    ...
    la("kerry", "Kerry", "Munster", "county", fmt="pdf",
       listing="https://www.kerrycoco.ie/finance/financial-documents/", value_kind="po_committed"),
    la("meath", "Meath", "Leinster", "county", fmt="pdf",
       listing="https://www.meath.ie/council/your-council/finance-and-procurement/tenders-and-contracts/payments-over-eu20000",
       value_kind="payment_actual", caveat="'payments over €20,000' → SPENT; curl fallback (Python TLS quirk)"),
    la("sligo", "Sligo", "Connacht", "county", fmt="pdf",
       listing="https://www.sligococo.ie/YourCouncil/Finance/ProcurementPurchasing/PurchasingActivity/",
       value_kind="po_committed", caveat="curl fallback (Python TLS quirk)"),
    # ---- one-hop crawl from the landing page ----
    la("clare", "Clare", "Munster", "county", fmt="pdf",
       listing="https://www.clarecoco.ie/business-licensing-and-economy/procurement-and-tenders", value_kind="po_committed",
       direct=["https://www.clarecoco.ie/sites/default/files/2025-08/purchase-orders-over-20-000-in-the-2nd-quarter-of-2025-58204.pdf"]),
    la("leitrim", "Leitrim", "Connacht", "county", fmt="pdf",
       listing="https://www.leitrim.ie/council/services/finance/accounts-payable/purchase-to-pay/",
       value_kind="po_committed", aggregate_guard=True,
       caveat="finance page also lists a prompt-pay AGGREGATE return → aggregate_guard skips 1-row files"),
    la("laois", "Laois", "Leinster", "county", fmt="pdf",
    ...
    la("fingal", "Fingal", "Dublin", "dublin", fmt="pdf",
       listing="https://www.fingal.ie/council/service/procurement", value_kind="po_committed", supplier_is_id=True,
    ...
    la("mayo", "Mayo", "Connacht", "county", fmt="pdf",
       listing="https://www.mayo.ie/financial-documents/purchase-orders", value_kind="po_committed", status="DIRECT",
       direct=["https://www.mayo.ie/getattachment/6a915b4f-ab78-4b69-8ba6-d07569222e03/attachment.aspx",
    ...
    la("carlow", "Carlow", "Leinster", "county", fmt="pdf", status="NEEDS-RENDER",
       listing="https://carlow.ie/information-technology/statistics-and-reports/financial-statistical-reports",
       value_kind="po_committed", caveat="JS/SPA file list — Playwright enumeration deferred"),
    la("cavan", "Cavan", "Ulster", "county", fmt="pdf", status="NEEDS-RENDER",
       listing="https://www.cavancoco.ie/file-library/business/procurement/over-20k/",
       value_kind="po_committed", caveat="JS-rendered file library — Playwright enumeration deferred"),
    la("roscommon", "Roscommon", "Connacht", "county", fmt="pdf", status="NEEDS-RENDER",
       listing="https://www.roscommoncoco.ie/en/Download-It/Finance-Publications/",
       value_kind="po_committed", caveat="JS-rendered download portal — Playwright enumeration deferred"),
    # ---- non-publishers (kept for the full 31 census; never parsed) ----
    la("dublin_city", "Dublin City", "Dublin", "dublin", fmt="-", status="NON-PUBLISHER",
       listing="https://www.dublincity.ie/business/doing-business-council/public-procurement",
       value_kind="po_committed", caveat="only a stale ≤2014 aggregate prompt-payment return; wrong grain"),
    la("dlr", "Dún Laoghaire-Rathdown", "Dublin", "dublin", fmt="-", status="NON-PUBLISHER",
       listing="https://www.dlrcoco.ie/governance/procurement", value_kind="po_committed",
...
POLICY_RE = re.compile(
    ...
    r"quality.?assurance|\bpsc\b|spending.?code|oversight|\bnoac\b|qa.?report", re.I)
DATA_FILE_RE = re.compile(r"q[1-4]\b|qtr|quarter|20[12]\d|over.?20|20k|purchase|payment|\bpo[s]?\b", re.I)
NAV_HINT = re.compile(
    r"purchase|procure|over.?20|20k|payment|quarter|qtr|finance|publication|spend|supplier", re.I)
...
def split_row(words: list) -> dict | None:
    ...
    if len(supplier) < 3:
        return None
    return {"supplier": supplier, "eur": eur, "description": description or None,
            "po": None, "paid": None}
...
def _col_roles(header: list[str], data: list[list]) -> dict[str, int | None]:
    ...
    for role, rx in (("supplier", SUP_RE), ("amount", AMT_RE), ("description", DESC_RE),
                     ("paid", PAID_RE), ("po", PO_RE)):
    ...
    if (roles["supplier"] is None or roles["amount"] is None) and data:
        ...
        if roles["supplier"] is None and any(txts):
            roles["supplier"] = max((j for j in range(ncol) if j != roles["amount"]),
                                    key=lambda j: txts[j], default=None)
    return roles
...
def _tabular_rows(grid: list[list]) -> tuple[list[dict], int]:
    ...
    def role_hits(row):
        return sum(bool(SUP_RE.search(str(c)) or AMT_RE.search(str(c)) or DESC_RE.search(str(c)))
                   for c in (row or []) if c is not None)
    ...
    for r in data:
        ...
        if eur is None or sup is None or not str(sup).strip():
            continue
        out.append({
            "supplier": strip_id_prefix(str(sup).strip()), "eur": eur,
...
def emit_file(cf: dict, file_url: str, b: bytes, ext: str) -> tuple[list[dict], dict]:
    ...
    for srn, rec in enumerate(recs):
        ...
        if eur < MIN_EUR:
            continue
        rows_out.append({
            "publisher_id": f"ie_la_{cf['slug']}", "publisher_name": cf["council"],
            "publisher_type": "local_authority", "sector": "local_government",
            "region": cf["region"], "entity_type": cf["entity"],
            "source_landing_url": cf["listing_url"], "source_file_url": file_url,
            "source_file_hash": fhash, "period": period, "year": year, "quarter": quarter,
            "supplier_raw": sup, "amount_eur": eur,
            "value_kind": cf["value_kind"], "realisation_tier": TIER[cf["value_kind"]],
            "description": rec["description"], "po_number": rec["po"], "paid_flag": rec["paid"],
            "source_row_number": srn, "source_page_number": rec["page"],
            "parser_name": f"la_payments_{ext.lstrip('.')}", "parser_version": PARSER_VERSION,
...
def classify_and_flag(df: pl.DataFrame) -> pl.DataFrame:
    ...
    df = df.with_columns(
        ...
        pl.col("supplier_raw").map_elements(lambda s: bool(ID_CODE_RE.match((s or "").strip())), return_dtype=pl.Boolean).alias("_idc"),
    ).with_columns(
        pl.when(pl.col("supplier_is_id_code") | pl.col("_idc")).then(pl.lit("id_code"))
        .when(pl.col("_pub")).then(pl.lit("public_body"))
        ...
        pl.when(pl.col("supplier_class") == "sole_trader_or_individual").then(pl.lit("sole_trader_or_individual"))
        .when(pl.col("supplier_class") == "id_code").then(pl.lit("bare_id_code"))
        .when(pl.col("supplier_class") == "unknown").then(pl.lit("unresolved_name"))
        .otherwise(pl.lit(None)).alias("privacy_reason"),
...
FACT_COLS = [
    "publisher_id", "publisher_name", "publisher_type", "sector", "region", "entity_type",
    "source_landing_url", "source_file_url", "source_file_hash",
    "period", "year", "quarter",
    "supplier_raw", "supplier_normalised", "supplier_class", "supplier_is_id_code",
    "amount_eur", "value_kind", "realisation_tier", "value_safe_to_sum",
    "description", "po_number", "paid_flag",
    "source_row_number", "source_page_number", "parser_name", "parser_version",
    "privacy_status", "public_display", "privacy_reason", "source_caveat",
]
...
def main() -> None:
    ...
    councils = [c for c in SCHEMA_MAP
                if (not only or c["slug"] in only)
                and c["status"] not in ("NON-PUBLISHER", "NEEDS-RENDER")]
    hr(f"LA PO/PAYMENTS-over-€20k EXTRACT — {len(councils)} councils"
       + (" (LIST MODE)" if args.list else ""))
    ...
    for cf in councils:
        ...
        for u in files[:args.max_files]:
            ...
            print(f"     -> {stat['file']:<48} rows={stat['rows']:<5} €{stat['total_eur'] / 1e6:>6.2f}m  {stat['period']}")

        # per-council CRO reconciliation band
        cro_rate = 0.0
        if c_rows:
            cdf = pl.DataFrame({"supplier_raw": [r["supplier_raw"] for r in c_rows]})
            sup = (cdf.with_columns(name_norm_expr("supplier_raw").alias("nn"))
                   .filter(pl.col("nn").str.len_chars() >= 4).unique(subset=["nn"]))
            hit = sup.join(cro, left_on="nn", right_on="name_norm", how="left") \
                     .filter(pl.col("company_num").is_not_null()).select("nn").n_unique()
            cro_rate = hit / max(1, sup.height)
        all_rows.extend(c_rows)
        per_council.append({
            "slug": cf["slug"], "council": cf["council"], "region": cf["region"],
            "value_kind": cf["value_kind"], "files_parsed": len(file_stats),
            "rows": len(c_rows), "total_eur": sum(s["total_eur"] for s in file_stats),
            "cro_rate": round(cro_rate, 3), "periods": sorted({s["period"] for s in file_stats if s["period"]}),
        })
        if c_rows:
            print(f"   == {cf['council']}: {len(c_rows):,} rows  €{sum(s['total_eur'] for s in file_stats) / 1e6:.1f}m  CRO {cro_rate:.0%}")
    ...
    print(f"quarantined (personal data, hidden): {quar.height:,} rows "
          f"({quar.height / df.height:.0%})")
    ...
    cov = {
        ...
        "value_kind_counts": {r["value_kind"]: r["len"]
                              for r in df.group_by("value_kind").len().iter_rows(named=True)},

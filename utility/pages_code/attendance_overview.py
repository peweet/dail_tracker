...
def _render_good_bad(ranking_df: pd.DataFrame, year: int, key_pfx: str) -> str | None:
    ...
    bottom = (
        non_min.sort_values(["rank_low", "attended_count"], ascending=[True, True])
        .head(_HALL_SIZE)
        .reset_index(drop=True)
    )
...
def _render_alltime(df: pd.DataFrame, year_counts_df: pd.DataFrame) -> str | None:
    ...
    with col_good:
        ...
        st.html('<p class="att-hall-subheading">Total days attended across entire record period</p>')
        for i, (_, row) in enumerate(top.iterrows()):
            name = str(row["member_name"])
            medal = _GOOD_MEDALS[i] if i < len(_GOOD_MEDALS) else ""
            cc, bc = st.columns([14, 1])
    ...
    with col_right:
        ...
        st.html('<p class="att-hall-subheading">Includes ministers — select a year for a filtered ranking</p>')
        for i, (_, row) in enumerate(bottom.iterrows()):
            name = str(row["member_name"])
...
def attendance_overview_page() -> None:
    ...
    # ── Year-specific view ─────────────────────────────────────────────────────
        ...
        if ranking_df.empty:
            ...
            today_str = datetime.date.today().isoformat()
            export_df = ranking_df[["member_name", "party_name", "constituency", "attended_count", "rank_high"]].rename(
                columns={"rank_high": "rank"}
            )
            export_button(
                export_df,

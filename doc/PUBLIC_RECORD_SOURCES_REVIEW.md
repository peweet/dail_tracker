...
- **State Boards register (membership.stateboards.ie + publicjobs annual reports):** **complements, doesn't duplicate, Iris.** Iris captures appointment *events*; the register is the *current roster + the public-body universe* (publicjobs cites 240+ agencies). That universe is exactly the missing spine behind every "Public Body Profile" join (procurement publisher ↔ C&AG audited-body ↔ AFS). Caveat: the probe got a thin 8 KB shell — **tractability unproven; needs a deeper probe** (XHR/JSON behind the portal, or fall back to the publicjobs annual-report PDFs which are born-digital). Join key = body name (alias table needed). PII: board members are public office-holders — low risk, but keep to **role + body + term**, no contact data.

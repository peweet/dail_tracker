...
def _overview() -> None:
    ...
    with col_l:
        ...
        if not most_lobbied.empty and {"full_name", "total_returns", "distinct_orgs"}.issubset(most_lobbied.columns):
            # REMOVE: gold/most_lobbied_politicians.parquet is already deduplicated and ORDER BY total_returns DESC
            # Replace with: top_ml = most_lobbied.head(20).reset_index(drop=True)
            top_ml = (
    ...
    with col_r:
        ...
        if not lobby_count.empty and {"lobbyist_name", "lobby_requests_count"}.issubset(lobby_count.columns):
            # REMOVE: gold/top_lobbyist_organisations.parquet replaces this groupby+sort
            # Replace with: top_lc = gold_top_lobbyist_orgs.head(20).reset_index(drop=True)
            top_lc = (
    ...
    if not policy.empty and {"public_policy_area", "return_count"}.issubset(policy.columns):
        ...
        # REMOVE: gold/policy_area_breakdown.parquet already ORDER BY return_count DESC — drop this sort
    ...
    if not clients.empty and {"client_name", "return_count"}.issubset(clients.columns):
        ...
        # REMOVE: gold/top_client_companies.parquet already ORDER BY return_count DESC — drop sort + head
    ...
    if not bilateral.empty and {"returns_in_relationship", "lobbyist_name", "full_name"}.issubset(bilateral.columns):
        ...
        # REMOVE: gold/bilateral_relationships.parquet already ranked — drop sort + head

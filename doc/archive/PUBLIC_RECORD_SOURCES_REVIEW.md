...
| **State Boards register (current roster + body universe)** | **✅ BUILT 2026-06-12** | high / med | `extractors/stateboards_roster_extract.py` → `v_stateboards_roster` / `v_stateboards_boards`; `stateboards` chain. Wikidata identities are **hand-curated** (`data/_meta/stateboards_wikidata_curated.csv`); auto name-matching removed after audit (~25% wrong-person) |

...
def normalise_df_td_name(df: pl.DataFrame, col_name: str) -> pl.Series:
    ...
    full_name = df.with_columns(
        ...
        .str.normalize(
            "NFD"
        )  # Normalize Unicode characters to their closest ASCII equivalent (e.g. "Ó Súilleabháin" → "O Suilleabhain")
        .str.replace_all(r"[\u0300-\u036f]", "")
        .str.replace_all(r"[\x27\u2018\u2019\u02BC\u02B9\u0060\u00B4\uFF07]", "")
        .str.replace_all(
            r"[^a-z\s]", ""
        )  # remove any remaining non-alphabetic characters (e.g. spaces, hyphens, etc.) as they cause issues with joining names across datasets
        .str.replace_all(
            r"^\s*(dr|prof|rev|fr|sr|mr|mrs|ms|miss|br)\s+", ""
        )  # remove honorifics as they cause issues with joining names across datasets (e.g. Dr. John Smith becomes John Smith, which is easier to match with the same name in another dataset that doesn't include the honorific)

"""
SANDBOX REFACTOR — attendance.py

Goal: identical CSV/parquet output to current attendance.py, but:
  - No module-level side effects (os.chdir, pd.concat at import time)
  - Wrapped in main() so it's importable + testable
  - Skips cleanly (exit 0) when ATTENDANCE_PDF_DIR is empty
  - Skips cleanly (exit 0) when no tables extract from PDFs

Validated by test_attendance_sandbox.py against the canonical
data/silver/aggregated_td_tables.csv + td_attendance_fact_table.csv +
parquet/td_attendance_fact_table.parquet.

DELETE THIS FILE after the validated refactor is copied into attendance.py.
"""
...
def _extract_pdf_tables(pdf_dir: Path) -> tuple[list[pd.DataFrame], str]:
    """Extract tables from every PDF in pdf_dir.

    Returns (dataframes, last_date_range). Identical extraction loop to the
    original attendance.py — same regexes, same per-page logic, same column
    inserts. The loop reads PDFs by absolute path (no os.chdir) so callers
    don't have their cwd silently mutated.
    """
...
def _build_silver_csv(dataframes: list[pd.DataFrame], csv_path: Path) -> None:
    """Concat → clean → dedup → write silver CSV. Identical transforms to original."""
...
def _build_fact_table(silver_csv: Path, fact_csv: Path, fact_parquet: Path) -> None:
    ...
    Re-read (rather than passing the in-memory df) is preserved from the
    original — keeps any dtype coercions to_csv → read_csv applies, so the
    fact table is byte-identical to the canonical output.
    """
...
def main() -> int:
    ...
    Exit codes:
        ...
        1 — unexpected failure (let exception propagate)
    """

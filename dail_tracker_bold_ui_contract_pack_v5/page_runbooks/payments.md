# Page Runbook — Payments

## Target

Page ID: `payments`  
Page file: `utility/pages_code/payments.py`  
User question: **What parliamentary payments were published for members, and how do they change by year?**
...
```text
v_payments_summary, v_payments_member_detail, v_payments_yearly_evolution, v_payments_sources
```
...
- `utility/page_contracts/payments.yaml`
...
```text
We are working on `payments` only.

Read only:
1. `CLAUDE.md`
2. `page_runbooks/payments.md`
3. `utility/page_contracts/payments.yaml`
4. `utility/page_contracts/_shared_ui_policy.yaml`
5. `utility/page_contracts/_interaction_patterns.yaml`
6. `utility/pages_code/payments.py`
...
```text
Create a bold UI redesign plan for `payments`.
...
```text
Update `utility/page_contracts/payments.yaml` only if the redesign plan requires contract changes.
...
```text
Implement the `payments` UI redesign.
...
```text
The redesign is still too close to the old `payments` page.

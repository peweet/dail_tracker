...
Use `border=True` on containers for visual grouping:
...
**Warning:** Tabs always render all content, even hidden tabs. Use conditional rendering (`if view == "tab2":`) for expensive content — see `optimizing-streamlit-performance`.
...
**Warning:** Expander content always renders, even when collapsed. Use `if st.toggle(...)` for expensive content.

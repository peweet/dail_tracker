...
Page ID: `votes`  
Page file: `utility/pages_code/votes.py`  
User question: **How did members vote in published divisions, and what official evidence supports the result?**
...
```text
v_vote_index, v_vote_result_summary, v_vote_member_detail, v_vote_sources, party_vote_breakdown, td_vote_summary
```
...
- show result summary
- show member votes
- filter by party/constituency/member/position where approved
...
- `utility/page_contracts/votes.yaml`
...
```text
We are working on `votes` only.

Read only:
1. `CLAUDE.md`
2. `page_runbooks/votes.md`
3. `utility/page_contracts/votes.yaml`
4. `utility/page_contracts/_shared_ui_policy.yaml`
5. `utility/page_contracts/_interaction_patterns.yaml`
6. `utility/pages_code/votes.py`
...
```text
Create a bold UI redesign plan for `votes`.
...
```text
Update `utility/page_contracts/votes.yaml` only if the redesign plan requires contract changes.
...
```text
Implement the `votes` UI redesign.

This is a UI redesign, not a safe refactor.

You may modify only the allowed files listed in `page_runbooks/votes.md`.
...
```text
The redesign is still too close to the old `votes` page.

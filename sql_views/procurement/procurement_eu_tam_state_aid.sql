...
SELECT
    ...
    date_granted,
    granting_authority,
    CASE WHEN lower(trim(entrusted_entity)) IN ('none', 'n/a') THEN NULL ELSE entrusted_entity END AS entrusted_entity,
    CASE WHEN lower(trim(financial_intermediary)) IN ('none', 'n/a') THEN NULL ELSE financial_intermediary END AS financial_intermediary,
    published_date,
    award_detail_url,
    value_kind,
    realisation_tier,

...
votes_df = votes_df.with_columns(pl.col('vote_id').str.split(by='_').list.slice(-1, 1).alias('vote_id')).explode('vote_id')
...
bill_date_data=pl.read_csv(DATA_DIR / "silver" / 'stages.csv', encoding='utf-8'
                           ).select([
                               'event.house.houseNo',
                               'bill.billNo', 
                               'bill.billYear', 
...
current_dail_vote_history_df = votes_df.join(key_data, 
                                             on='unique_member_code', 
                                             how='left'
                                             ).join(bill_date_data, 
                                                    left_on=['debate_title'], 
                                                    suffix='_test',right_on=['short_title'], 
                                                    how='left')
current_dail_vote_history_df = current_dail_vote_history_df.unique(
    subset=['unique_member_code', 'vote_id']).drop('join_key')

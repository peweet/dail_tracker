...
LAYER_REGISTRY = [
    dict(
        key="npws_sac", role="ecology / Appropriate Assessment (#12-13)",
        url="https://services-eu1.arcgis.com/Jhij7i46ouO8Cc0N/arcgis/rest/services/NPWSDesignatedAreas/FeatureServer/3",
        geom="esriGeometryPolygon", sr=2157, required_fields=["SITECODE", "SITE_NAME"],
        licence="gov open", approx_count=433, freshness="lastEditDate exposed (2026-04-01)",
    ),
    dict(
        key="gsi_gw_vuln", role="septic / Site Suitability (#25)",
        ...
        geom="esriGeometryPolygon", sr=2157, required_fields=["VUL_CAT", "VUL_DESC"],
        licence="CC-BY-4.0", approx_count=221_148, freshness="NO lastEditDate -> must poll/hash",
    ),
    dict(
        key="nms_smr_points", role="archaeology (#17)",
        url="https://services-eu1.arcgis.com/HyjXgkV6KGMSF3jt/arcgis/rest/services/SMROpenData/FeatureServer/0",
        geom="esriGeometryPoint", sr=2157, required_fields=["SMRS", "MONUMENT_CLASS"],
        licence="gov open", approx_count=151_308, freshness="lastEditDate 2026-06-04",
    ),
    dict(
        key="nms_smr_zone", role="archaeology buffer (#17) — join ZONE_ID to SMR points",
        url="https://services-eu1.arcgis.com/HyjXgkV6KGMSF3jt/arcgis/rest/services/SMRZoneOpenData/FeatureServer/0",
        geom="esriGeometryPolygon", sr=2157, required_fields=["ZONE_ID"],
        licence="gov open", approx_count=81_409, freshness="lastEditDate 2026-06-04",
    ),
    dict(
        key="planning_apps", role="context register (495k)",
        url="https://services.arcgis.com/NzlPQPKn5QF9v2US/arcgis/rest/services/IrishPlanningApplications/FeatureServer/0",
        geom="esriGeometryPoint", sr=3857, required_fields=["Decision", "PlanningAuthority"],
        licence="CC-BY-4.0", approx_count=495_632, freshness="lastEditDate 2026-06-09",
    ),
    dict(
        key="myplan_zoning", role="material contravention (§10.6) — RESOLVED endpoint",
        ...
        geom="esriGeometryPolygon", sr=2157,
        required_fields=["ZONE_GZT", "ZONE_ORIG", "ZONE_DESC"],  # ZONE_GZT nullable; ZONE_ORIG/DESC populated
        licence="gov open (MyPlan/DHLGH)", approx_count=82_664,
        freshness="lastEditDate 2026-05-13; carries PLAN_FROM/PLAN_TO/CURRENT_PLAN (temporal versioning)",
    ),
    dict(
        key="niah", role="heritage / architectural (#16) — NATIONAL",
        url="https://services-eu1.arcgis.com/HyjXgkV6KGMSF3jt/arcgis/rest/services/NIAHBuildingsOpenData/FeatureServer/0",
        geom="esriGeometryPoint", sr=2157, required_fields=["REG_NO", "NAME", "ORIGINAL_TYPE"],
        licence="gov open (DHLGH)", approx_count=48_327, freshness="lastEditDate 2025-04-03",
    ),
]


def _probe(url):
    return requests.get(url, params={"f": "json"},
                        headers={"User-Agent": "dail-tracker-test/1.0"}, timeout=45).json()
...
@pytest.mark.parametrize("layer", LAYER_REGISTRY, ids=lambda l: l["key"])
def test_layer_available_geometry_and_required_field(layer):
    ...
    assert any(rf.upper() in fnames for rf in layer["required_fields"]), (
        f"{layer['key']} MISSING all required fields {layer['required_fields']}; "
        f"live schema has: {sorted(fnames)}")


@pytest.mark.skipif(not ONLINE, reason="offline")
@pytest.mark.parametrize("layer", LAYER_REGISTRY, ids=lambda l: l["key"])
def test_layer_has_rows(layer):
    c = requests.get(layer["url"] + "/query",
...
def test_myplan_onprem_endpoint_is_dead():
    ...
    with pytest.raises(_rq.exceptions.RequestException):
        _rq.get("https://maps.housing.gov.ie/arcgis/rest/services/MyPlan/GZTZoningEPSG2157/MapServer/0",

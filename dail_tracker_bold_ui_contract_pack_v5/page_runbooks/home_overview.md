# Page Runbook — Dáil Tracker

## Target

Page ID: `home_overview`  
Page file: `utility/pages_code/home_overview.py`  
User question: **What does Dáil Tracker cover, and where should a citizen or journalist start?**
...
```text
v_home_dataset_status, v_home_recent_updates, v_home_featured_metrics
```
...
- `utility/page_contracts/home_overview.yaml`
...
```text
We are working on `home_overview` only.

Read only:
1. `CLAUDE.md`
2. `page_runbooks/home_overview.md`
3. `utility/page_contracts/home_overview.yaml`
4. `utility/page_contracts/_shared_ui_policy.yaml`
5. `utility/page_contracts/_interaction_patterns.yaml`
6. `utility/pages_code/home_overview.py`
...
```text
Create a bold UI redesign plan for `home_overview`.
...
```text
Update `utility/page_contracts/home_overview.yaml` only if the redesign plan requires contract changes.
...
```text
Implement the `home_overview` UI redesign.

This is a UI redesign, not a safe refactor.

You may modify only the allowed files listed in `page_runbooks/home_overview.md`.
...
```text
The redesign is still too close to the old `home_overview` page.

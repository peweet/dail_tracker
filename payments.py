...
def process_payment_pdfs():
    ...
    cleaned_rows = []
    for row in all_rows:
        if not EXCLUDE_PLACEHOLDER.search(
            str(row[0])
        ):  # exclude rows that contain placeholder text like "Parliamentary Standard" in the first column, which are not actual payment records but just headers or footers in the PDF
            cleaned_rows.append(row)
    all_rows = cleaned_rows
    print(f"Total rows extracted: {len(all_rows)}")
    df = pl.DataFrame(all_rows, schema=["Name", "TAA_Band", "Narrative", "Date_Paid", "Amount"], orient="row")
    df = df.with_columns(pl.col("Name").str.splitn(by=" ", n=2).alias("Name_Split"))
    df = (
        df.with_columns(pl.col("Name_Split").struct.rename_fields(["Position", "Full_Name"]))
        .unnest("Name_Split")
        .drop("Name")
    )
    df = df.with_columns(
        pl.when(pl.col("Date_Paid").str.contains("/")).then(pl.col("Date_Paid")).otherwise(None).alias("Date_Paid")
    )
    ...
    top_tds_by_payment = top_tds_by_payment.filter(pl.col("Amount").is_not_null() & (pl.col("Amount") < 10_000))
    top_tds_by_payment = top_tds_by_payment.with_columns(
        pl.sum("Amount").over("join_key").alias("total_amount_paid_since_2020").round(2)
    ).sort(["Date_Paid", "total_amount_paid_since_2020"], descending=True)
...
if __name__ == "__main__":
    ...
    print(
        "Payment PDF processing complete. Output saved to aggregated_payment_tables.csv and top_tds_by_payment_2020.csv."
    )

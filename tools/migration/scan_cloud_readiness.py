...
def runtime_resilience() -> dict:
    ...
    for path in sorted(files):
        ...
        if GOVIE_IN_CODE_RE.search(src) and not BROWSER_UA_RE.search(src):
            if BARE_REQUEST_RE.search(src) or FETCH_ENGINE_RE.search(src):
...
def render(sources: list[dict]) -> str:
    ...
    w("> Generated by `python tools/scan_cloud_readiness.py`. A measurement, not a "
    ...
    w("## Headline\n")
    w(f"- **{high} sources** sit behind a HIGH-risk host (gov.ie WAF) — these can "
    ...
    w(f"- **{blockers} runtime modules** hardcode a `c:/tmp` or Windows path — hard "
      f"blockers on Linux.")
    w(f"- **{len(res['bare_requests'])} runtime modules** bypass the resilient "
      f"`fetch_bytes` engine with bare `requests`.")
    w(f"- Polling: **{pol['pollable']}/{pol['total']}** sources pollable; but only "
      f"**{pol['wired']}** have a wired parser.\n")

    # ---- axis 1 ----
    w("## Axis 1 — source host exposure\n")
    w("Each source graded by its worst host. WAF_GOVIE is the axis that actually "
      "decides which runner a source can live on.\n")
    w("| Risk | Sources | Meaning |")
    w("|---|---:|---|")
    w(f"| HIGH (gov.ie WAF) | {exp['band_totals'].get('HIGH', 0)} | datacenter ASN may block even spoofed → residential/self-hosted runner only |")
    ...
    w("A source with no poller and no canary goes stale silently once nobody is "
      "watching the laptop.\n")
    w("| Measure | Count |")
    w("|---|---:|")
    ...
    w(f"Scanned {res['n_files']} modules in `extractors/` + `iris/`. "
    ...
    w("Each of these writes or reads a Windows/`c:/tmp` path that does not exist on "
    ...
    w("Bare `requests`/`urlopen`: no shared-session retry, no curl fallback, no "
    ...
    if res["govie_no_ua"]:
        ...
        w("These touch a gov.ie host but the module shows no browser-UA spoof. Verify "
    ...
    w("1. **HIGH-risk sources decide the runner.** As long as any gov.ie-WAF source "
    ...
    w("2. **The c:/tmp blockers are the migration checklist.** Every module above must "
    ...
    w("3. **The bare-requests list is the resilience backlog.** Route file downloads "
      "through `fetch_bytes` so a WAF 403 degrades to a curl fallback instead of "
      "crashing the chain.\n")
    w("4. **Polling coverage is your only cloud early-warning.** Sources that are not "
...
def main() -> int:
    ...
    if args.json:
        ...
        payload = {
            ...
            "polling": {k: (dict(v) if isinstance(v, Counter) else v)
                        for k, v in polling_coverage(sources).items()},
            "resilience": {
                k: (v if isinstance(v, (int, list)) else v)
                for k, v in runtime_resilience().items()
            },
        }

...
from config import API_BASE, BRONZE_DIR, DATA_DIR, LEGISLATION_DIR, LOG_DIR, MEMBERS_DIR, VOTES_DIR
...
def fetch_votes():
    ...
    votes_query1 = (
        f"{API_BASE}/votes?chamber_type=house&chamber_id=&chamber=dail&date_start=2020-01-01&limit=10000&outcome="
    )
    votes_query2 = f"{API_BASE}/votes?chamber_type=house&chamber_id=&chamber=dail&date_start=2020-01-01"
    votes_query3 = (
        f"{API_BASE}/votes?chamber_type=house&chamber_id=&chamber=dail&date_end=2019-12-31&limit=10000&outcome="
    )
...
if __name__ == "__main__":
    ...
    logger.info("  - data/bronze/members/members.json")
    logger.info("  - data/bronze/legislation/legislation_results.json")
    logger.info("  - data/bronze/legislation/questions_results.json")
    logger.info("=" * 70)
    logger.info("=" * 70)

# Page Runbook — Register of Members' Interests

## Target

Page ID: `interests`  
Page file: `utility/pages_code/interests.py`  
User question: **What interests has this member declared, and how have those declarations changed over time?**
...
```text
v_member_interests_index, v_member_interests_detail, v_member_interests_yearly_summary, v_member_interests_sources
```
...
- `utility/pages_code/interests.py`
...
- `utility/page_contracts/interests.yaml`
...
```text
We are working on `interests` only.

Read only:
1. `CLAUDE.md`
2. `page_runbooks/interests.md`
3. `utility/page_contracts/interests.yaml`
4. `utility/page_contracts/_shared_ui_policy.yaml`
5. `utility/page_contracts/_interaction_patterns.yaml`
6. `utility/pages_code/interests.py`
...
```text
Create a bold UI redesign plan for `interests`.
...
```text
Update `utility/page_contracts/interests.yaml` only if the redesign plan requires contract changes.
...
```text
Implement the `interests` UI redesign.

This is a UI redesign, not a safe refactor.

You may modify only the allowed files listed in `page_runbooks/interests.md`.
...
```text
The redesign is still too close to the old `interests` page.

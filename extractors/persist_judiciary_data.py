...
import fitz  # PyMuPDF (in .venv)  # noqa: E402
...
spine = (
    jud.with_columns(pl.col("body").is_in(REAL).alias("is_real_court"))
    .select(
        [
            "notice_ref",
            "issue_date",
...
GOVIE = [
    ...
    (
        ...
        "Edward Carroll",
        "District Court",
        "Cork solicitor (partner since 1995)",
        "resignation",
        "Judge O'Shea (Jul 2025)",
    ),
    (
        "2026-04-01",
        "Paula Cullinane",
        "District Court",
        "Dublin solicitor (partner since 2021)",
    ...
    (
        ...
        "Andrew Gubbins",
        "District Court",
        "Tipperary barrister (called 2005)",
        "Circuit Court elevation",
        None,
    ),
    (
        ...
        "Tom MacSharry",
        "District Court",
        "Sligo solicitor (sole practitioner since 2007)",
        "Circuit Court elevation",
        None,
    ),
    ...
    (
        ...
        "Darach McCarthy",
        "District Court",
        "Limerick solicitor (principal since 2006)",
        "retirement",
        None,
    ),
    ...
    (
        ...
        "Olivia Traynor",
        "District Court",
        "Galway solicitor (sole practitioner since 2012)",
        "retirement",
        None,
    ),
    (
        ...
        "Senior Counsel",
        "elevation",
        "A.M. Owens elevated to Court of Appeal (29 Sep 2025)",
    ),
    (
        "2025-12-18",
        "Micheál O'Connell",
        ...
        "Senior Counsel",
        "retirement",
        "Judge Mary Ellen Ring (27 Dec 2025)",
    ),
    ...
    (
        ...
        "Anthony Collins",
        "Court of Appeal",
        "former Advocate General (CJEU)",
        "new post (2024 legislation)",
        None,
    ),
    (
        "2024-10-31",
        ...
        "Senior Counsel",
        "elevation",
        "N. Hyland elevated to Court of Appeal (3 Oct 2024)",
    ),
    ...
    (
        ...
        "Solicitor and BL",
        "retirement",
        "Judge Marian O'Leary (30 Aug 2024)",
    ),
]
govie = pl.DataFrame(
    GOVIE,
    schema=["announce_date", "nominee", "target_court", "prior_career", "vacancy_cause", "predecessor"],
    orient="row",
)
...
ASSIGN = [
    ...
    ("Hunt", "Special Criminal Court"),
    ("Coffey", "Personal Injuries List (incl. Clinical Negligence)"),
    ("Gearty", "Non-Jury / Judicial Review and Asylum, Immigration and Citizenship Lists"),
    ("Cregan", "Chancery List"),
    ("Sanfey", "Commercial List"),
    ("Humphreys", "Planning and Environment List"),
    ("Jackson", "Family List"),
    ("Jordan", "Minors in Special Care List"),
    ("O'Connor", "Civil Jury List"),
    ("Cahill", "Personal Insolvency List"),
    ("Barrett", "Competition List"),
    ("O'Donnell", "Hague Luxembourg Convention List"),
    ("Stack", "Probate List"),
    ("Reynolds", "Garda Compensation List"),
    ("Kennedy", "Bankruptcy List and Examiner's List"),
    ...
    ("Mulcahy", "Admiralty matters"),
    ("O'Regan", "Land Registry matters"),
]
...
clr_files = {
    2017: TMP / "ca_2017.csv",
    2020: TMP / "ca_2020.csv",
    2022: TMP / "ca_2022.csv",
    2023: TMP / "ca_2023.csv",
    2024: TMP / "courts_annual_2024.csv",
}
frames = []
for _y, f in clr_files.items():
            ...
            for r in tab.extract():
                ...
                if len(c) >= 3 and c[0] and wk.search(" ".join(c[-2:]).lower()):
                    ...
                    if h not in ("nature of application", "venue") and "waiting time" not in h:
                        out.append(
                            {"page": p, "matter_or_venue": c[0][:80], "wait_2024": c[-2][:30], "wait_2023": c[-1][:30]}
                        )
    doc.close()
    return out
...
SAL = [
    ("Chief Justice", 266295),
    ("President of the High Court", 247276),
    ("Ordinary Judge, Supreme Court", 232060),
    ("President of the Circuit Court", 224452),
    ("Ordinary Judge, High Court", 218748),
    ("President of the District Court", 171327),
    ("Judge, Circuit Court", 165421),
    ("Judge, District Court", 147961),
]
...
EURO = [
    ("Cearbhall Ó Dálaigh", "CJEU"),
    ("John L. Murray", "CJEU"),
    ("Fidelma Macken", "CJEU"),
    ("Aindrias Ó Caoimh", "CJEU"),
    ("Eugene Regan", "CJEU"),
    ("Tom O'Higgins", "CJEU"),
    ("Kevin O'Higgins", "CJEU"),
    ("Conor Maguire", "ECtHR"),
]

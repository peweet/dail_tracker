...
def test_uv_executable_accepts_an_explicit_install(monkeypatch, tmp_path: Path) -> None:
    executable = tmp_path / "custom-uv.exe"
    executable.write_bytes(b"uv")
    monkeypatch.setenv(dev_env.UV_EXECUTABLE_VAR, str(executable))
    monkeypatch.setattr(dev_env.shutil, "which", lambda command: None)

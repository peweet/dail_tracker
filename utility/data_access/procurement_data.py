"""Procurement (eTenders) data access — thin framework-neutral cached wrapper over dail_tracker_core.
...
only framework-neutral caching (``cache_resource`` for the connection,
``cache_data`` for per-query memoisation) and unwraps ``QueryResult`` to the

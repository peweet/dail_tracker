...
description: Optimizing Streamlit app performance. Use when apps are slow, rerunning too often, or loading heavy content. Covers caching, fragments, forms, conditional rendering, and static vs dynamic widget choices.
license: Apache-2.0
...
**Important:** Caches without `ttl` or `max_entries` can grow indefinitely and cause memory issues.
...
# GOOD: Bounded with max entries
@st.cache_data(max_entries=100)
def get_user_data(user_id):
    return fetch_user(user_id)
```

Use `ttl` for time-based expiration OR `max_entries` for size-based limits. Usually don't need both.
...
```python
# BAD: New cache entry per filter value, expensive load inside
@st.cache_data
...
```python
# BAD: Full app reruns on every button click
...
Use `border=False` for seamless inline forms:
...
> **Note:** This is an escape hatch — prefer a database that queries on demand.

...
def _render_rule_expander(issue, label: str = "What the Development Plan says (verbatim)") -> None:
    """The collapsed verbatim Development-Plan rule + required-report list for one issue."""
    rule = issue.rule
    if not (rule and (rule.dm_standards or rule.checklist)):
        return
...
def _render_standard(issue) -> None:
    """Compact card for a STANDARD requirement (applies to ~every application of this type).
...
def siting_check_page() -> None:
    ...
    # TIERED rendering (tier_issues is the shared core logic, so this never diverges from the engine
    # /MCP). The fix for the over-flagging "everywhere is a floodplain/bat survey": separate the few
    # things notable about THIS location from the standard requirements every application of this type
    # gets, and from the checks that depend on a layer we can't read / features we can't see.
    tiers = tier_issues(res)

    # 1) SITE-SPECIFIC — the real signal (sorted F > D > P).
    site = sorted(tiers.site_specific, key=lambda i: "FDP".index(_headline_class(i.mitigation_classes)))
    if site:
        st.subheader(f"{len(site)} issue(s) specific to this location")
        for issue in site:
            _render_card(issue)
    ...
    if tiers.access is not None:
        st.subheader("Access & entrance")
        _render_card(tiers.access)

    # 3) STANDARD REQUIREMENTS — apply to essentially every application of this type; de-emphasised in
    # an expander so they don't drown the site-specific signal (but still shown, incl. the rural gate).
    if tiers.standard:
    ...
    # 4) CHECKS TO RUN YOURSELF — licensed/unreadable layers (OPW flood) + site-feature-dependent

...
def _fetch_acp() -> pl.DataFrame:
    ...
    while True:
        r = requests.get(
            ...
            params={
                "where": "1=1",
                "outFields": "ABPCASEID,DECISION,PLANINGATY,CATEGORY,DECIDED_ON",
...
def main() -> None:
    ...
    apps = pl.read_parquet(SILVER).select(
        "ApplicationNumber", "PlanningAuthority", "decision_normalised", "AppealRefNumber"
    )
    ...
    apps = apps.with_columns(
        ...
        .alias("abp_case"),
        pl.when(pl.col("decision_normalised").is_in(["Granted", "Granted-Conditional"]))
        .then(pl.lit("GRANT"))
        .when(pl.col("decision_normalised") == "Refused")
        .then(pl.lit("REFUSE"))
    ...
    LOG.info(
        "clear-vs-clear appeals: %d | ABP OVERTURNED council: %d (%.1f%%) [grant->refuse %d, refuse->grant %d] | upheld %d",
        n,
        rev,
        100 * rev / n,
        g2r,

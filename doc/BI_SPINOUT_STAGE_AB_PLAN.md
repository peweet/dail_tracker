...
| **Cross-register footprint** | BUILT — `lobbying_overlap`, `charity_overlap`, `epa_compliance_for_supplier`, `xref_summary`. **Ministerial-diary access is EXCLUDED from paid reports** (free civic only, reversed 2026-07-08 — [architecture §4](BI_SPINOUT_ARCHITECTURE.md)). Lobbying overlap is per-report/owner-gated, reports-only (never export/API), raw counts only, award-€ kept out of the lobbying table, `PROC_LOBBY` verbatim | Lobbying overlap is supplier-keyed only; **no diary / `who_ministers_meet` panel in a paid buyer report**. Any buyer-level cross-register aggregation = **PHASE-2-TODO** | **PHASE-2-TODO** for any per-category aggregation (needs a category-level cross-register view); otherwise restrict to the per-supplier co-presence rows already produced by the BUILT supplier overlaps — **no** per-category rollup in the report (firewall) |
...
**(d) The resolved decisions (2026-06-28, ministerial-diary half reversed 2026-07-09 after the Fable assessment).** (i) **Reports-first** — hand-built reports for 3–5 paying partners before any accounts/alerts/persistence (Phase 5, owner-gated). (ii) **Ministerial-diary access is OUT of the paid product entirely** (free civic only — [architecture §4](BI_SPINOUT_ARCHITECTURE.md), [Fable §7](BI_SPINOUT_FABLE_ASSESSMENT.md)). No `who_ministers_meet` / `ministerial_diary_organisation` panel on any supplier, buyer, or category paid report. (iii) **Lobbying-overlap may stay paid — tightly gated:** raw lobbying counts as separate labelled facts, never a score; **the company is the subject, never the politician**; **per-report/owner-gated and reports-only — never in bulk exports or the API**; the **award-€ figures must not sit in the same table/line as the lobbying count**; carry `PROC_LOBBY` verbatim; link out to the free public register; company-class/PII double-gate applies; flagged highest reputational risk.
   - (iv) **Iris-derived corporate distress is in the paid product — FACT-ONLY + ATTRIBUTED:** facts only (notice type, date, entity, status), never verbatim gazette text or PDF layout; carry the Iris acknowledgement + URL on every use; **confirm-before-first-sale** and **reports-only (never export/API)**; remains a solicitor-checklist item (expanded to sui generis DB right + site terms).

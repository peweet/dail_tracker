...
Covers the candidate-summary (Part 3) table produced by sipo_expenses_paddle_etl.py:
  - data/silver/sipo/sipo_expenses_fact.parquet   (combined, all parties)
...
Two layers in one file (same shape as test_procurement_data_quality.py):
  1. pytest invariant tests  — hard pass/fail data contracts. SKIPS if the parquet is
     absent (pre-promotion / regenerable). Run:
       ./.venv/Scripts/python.exe -m pytest test/test_sipo_data_quality.py -q
...
doc/archive/SIPO_OCR_REMAINING_QUEUE.md): every constituency is in the closed set of 43; spend and
...
FACT = ROOT / "data/silver/sipo/sipo_expenses_fact.parquet"
...
# -------------------------------------------------------------------- Part 4 tests
ITEMS = ROOT / "data/silver/sipo/sipo_expense_items_fact.parquet"
CATS = ROOT / "data/silver/sipo/sipo_expense_categories_fact.parquet"

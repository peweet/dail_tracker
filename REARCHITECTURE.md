...
### 11. DuckDB as the analytical layer — interview-ready SQL practice

DuckDB reads CSV and Parquet files directly with zero setup. This makes it ideal for practising the kind of SQL that comes up in data engineering interviews: window functions, CTEs, self-joins, conditional aggregation, and analytical patterns.

#### Setup

```python
import duckdb
con = duckdb.connect('gold/dail.duckdb')

# Load CSVs into persistent tables (one-time)
con.execute("CREATE TABLE td AS SELECT * FROM 'members/enriched_td_attendance.csv'")
con.execute("CREATE TABLE bills AS SELECT * FROM 'bills/drop_cols_flattened_bills.csv'")
# Once questions are flattened:
# con.execute("CREATE TABLE questions AS SELECT * FROM 'questions/flattened_questions.csv'")
```
...
### 12. Additional API enrichment and joins

#### Divisions (votes) — `/v1/divisions`

The single most impactful enrichment you haven't tapped. Each division record contains:

- The vote subject (bill title, motion)
- Date and chamber
- **Individual TD votes** — `tpisTá` (yes), `tpisNíl` (no), `tpisStaon` (abstain)

This enables:

- **Party loyalty score** — `COUNT(voted_with_party) / COUNT(all_votes)` per TD
- **Rebellion detection** — TDs who vote against their party whip
- **Absenteeism on votes** — TDs who are "attending" (PDF says present) but not voting (no division record). This gap is a more nuanced engagement metric than raw attendance.
- **Cross-party alliances** — Which opposition TDs vote with the government on specific issues?

Join strategy: `divisions.member_uri → td.unique_member_code` (both are the Oireachtas member ID)

#### Debates (speech records) — `/v1/debates`

Each debate record contains sections with individual speeches. Flattening gives:

- **Speech count per TD per debate** — who actually speaks vs sits silently
- **Word count / speech length** — volume of contribution
- **Topic tagging** — which TDs dominate which policy areas (housing, health, finance)
- **Question-to-speech ratio** — some TDs only ask questions, others make statements

Join strategy: debate sections contain `by.showAs` (TD name) and `by.memberCode` — join on member code.
...
#### External enrichment — CSO + GeoHive

- **CSO PxStat** (`data.cso.ie`) — constituency population, unemployment, age demographics. Join on constituency name. Answers: "Do TDs from high-unemployment constituencies attend more?"
- **GeoHive** — geographic boundaries for constituencies. Enables map visualisations (choropleth of attendance by constituency).
- **Electoral data** — vote share by TD. Cross-reference with attendance: do TDs with slim margins attend more (fear of losing seat)?

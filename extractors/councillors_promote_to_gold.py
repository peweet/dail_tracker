...
def _mdate(fn: str) -> str:
    ...
    m = re.search(r"(\d{1,2})(?:st|nd|rd|th)?\s+(January|February|March|April|May|June|July|August|"
                  r"September|October|November|December)\s+(20\d{2})", fn, re.I)
...
def main() -> int:
    ...
    # 1. roster
    roster = list(csv.DictReader(open(SBX / "councillors_roster.csv", encoding="utf-8")))
    _write("la_councillors.csv", ["local_authority", "lea", "name", "party", "status", "source"],
           [{k: r.get(k, "") for k in ["local_authority", "lea", "name", "party", "status", "source"]} for r in roster])

    # 2. coverage tiers
    cov = list(csv.DictReader(open(SBX / "council_coverage.csv", encoding="utf-8")))
    _write("la_council_meeting_coverage.csv",
           ["local_authority", "tier", "clean_minutes", "roster_councillors", "has_votes"],
           [{k: r.get(k, "") for k in ["local_authority", "tier", "clean_minutes", "roster_councillors", "has_votes"]} for r in cov])

    # 3. named votes (per councillor)
    votes = _jsonl(SBX / "member_votes.jsonl")
    _write("la_councillor_votes.csv", ["local_authority", "member", "meeting_date", "motion", "vote"],
           [{"local_authority": v["local_authority"], "member": v["member"],
             "meeting_date": _mdate(v.get("meeting", "")), "motion": (v.get("motion") or "")[:300],
             "vote": v["vote"]} for v in votes])

    # 4. meeting agendas (flatten agenda_items)
    mh = _jsonl(SBX / "meeting_history.jsonl")
    _write("la_meeting_agendas.csv", ["local_authority", "meeting_date", "agenda", "source_url"],
           [{"local_authority": r["council"], "meeting_date": r.get("date", ""),
             "agenda": SEP.join(r.get("agenda_items", [])), "source_url": r.get("source_url", "")}
            for r in mh if r.get("agenda_items")])

    # 5. standing orders (flatten order_of_business)
    so = _jsonl(SBX / "standing_orders.jsonl")
    _write("la_standing_orders.csv",
           ["local_authority", "order_of_business", "notice_of_motion", "voting", "quorum",
            "records_named_votes", "source_url"],
           [{"local_authority": r["local_authority"],
             "order_of_business": SEP.join(r.get("order_of_business", [])),
             "notice_of_motion": r.get("notice_of_motion", ""), "voting": r.get("voting", ""),
             "quorum": r.get("quorum", ""), "records_named_votes": r.get("records_named_votes", False),
             "source_url": r.get("source_url", "")} for r in so if r.get("status") == "ok"])

...
def _locate(src: str, name: str) -> tuple[int, int] | dict:
    ...
    if isinstance(node, ast.Name):  # assignment target — exact position on the node
        return node.lineno, node.col_offset
    # def/class: the identifier sits after the keyword on the def line
    col = src.splitlines()[node.lineno - 1].find(parts[-1], node.col_offset)
    return node.lineno, (col if col >= 0 else node.col_offset)

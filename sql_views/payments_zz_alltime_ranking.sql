...
latest_band AS (
    ...
    -- to v_payments_yearly_evolution on (member_name, latest_year).
    SELECT DISTINCT ON (y.member_name)
        y.member_name,
...
SELECT
    ...
    pm.earliest_year,
    pm.latest_year,
    RANK() OVER (ORDER BY pm.total_paid_since_2020 DESC)         AS rank_high
FROM per_member pm
LEFT JOIN latest_band lb ON pm.member_name = lb.member_name
